...
export class SocketIOWrapper {
  ...
  _init() {
    ...
    try {
      // 初始化SocketIO实例
      this.socketInstance = IO(import.meta.env.VITE_GLOB_SOCKET_NSP, {
        path: import.meta.env.VITE_GLOB_SOCKET_PATH,
      ...
      // error
      this.socketInstance.io.on(SocketIOWrapper.staticEvents[13], () => {
        console.log('已与服务端断开连接');
      });
    } catch (error) {
      console.log(error);
    }
